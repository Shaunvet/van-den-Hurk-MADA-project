...
```{r}
## ---- temporary_rf_runtime_log, message=TRUE, warning=FALSE -------------------

# Optional: Log run time and session info for audit trail
start_time <- Sys.time()
Sys.sleep(1)  # simulate runtime block
end_time <- Sys.time()

writeLines(as.character(end_time - start_time), here::here("results", "output", "rf_runtime_duration.txt"))
writeLines(capture.output(sessionInfo()), here::here("results", "output", "rf_session_info.txt"))

message("⏱️ Runtime and session info saved.")
...
```{r}
## ---- temporary_lasso_runtime_log, message=TRUE, warning=FALSE -------------------

start_time <- Sys.time()
Sys.sleep(1)  # Simulated delay
end_time <- Sys.time()

writeLines(as.character(end_time - start_time), here::here("results", "output", "lasso_runtime_duration.txt"))
writeLines(capture.output(sessionInfo()), here::here("results", "output", "lasso_session_info.txt"))

message("⏱️ LASSO runtime and session info saved.")
