...
tuned_path     <- here::here("results", "large-files", "rf_tuned.rds")
...
```
```{r}
## ---- fit_or_load_rf_model, message=TRUE, warning=FALSE -------------------

use_slim_model <- FALSE # Toggle this to FALSE to regenerate slim model if needed

slim_rf_path <- here::here("results", "output", "rf_model_minimal.rds")
full_rf_path <- here::here("results", "large-files", "rf_tuned.rds")
full_rf_metrics_path <- here::here("results", "output", "rf_tuned_metrics.rds")
best_rf_params_path <- here::here("results", "output", "rf_best_params.rds")

if (use_slim_model) {
  if (!file.exists(slim_rf_path)) {
    stop("Slimmed model not found. Please run with use_slim_model <- FALSE first to generate it.")
  }

  message("Loading slimmed RF model...")
  rf_slim <- readRDS(slim_rf_path)
  rf_final_model <- rf_slim$model
  rf_metrics <- rf_slim$metrics
  rf_vip <- rf_slim$vip
  best_rf_params <- readRDS(best_rf_params_path)

} else {
  if (!file.exists(full_rf_path)) {
    stop("Full Random Forest model not found. Please rerun the tuning block.")
  }

  message("Loading full RF model and generating slim version...")
  rf_tuned <- readRDS(full_rf_path)
  rf_metrics <- readRDS(full_rf_metrics_path)
  rf_final_model <- extract_workflow(rf_tuned)
...
# File paths
lasso_tuned_path <- here::here("results", "large-files", "lasso_tuned.rds")
lasso_metrics_path <- here::here("results", "output", "lasso_tuned_metrics.rds")
lasso_best_param_path <- here::here("results", "output", "lasso_best_params.rds")

# Load or tune
if (file.exists(lasso_tuned_path) && file.exists(lasso_metrics_path) && file.exists(lasso_best_param_path)) {
  ...
  message("LASSO Tuning complete and results saved.")
}
...
Creeating a slimmed down version of the tuning RDS file to reduce file size for Github and working with if statements to load this from the correct folder and to not overwrite files.
```{r}
## ---- slim_lasso_model_save, message=TRUE, warning=FALSE -------------------

slim_lasso_path <- here::here("results", "output", "lasso_model_minimal.rds")

if (!file.exists(slim_lasso_path)) {
  message("Creating slimmed LASSO model object...")

  lasso_final <- extract_workflow(lasso_tuned)
  lasso_metrics <- lasso_tuned_metrics

  # Extract coefficients
  lasso_fit <- extract_fit_parsnip(lasso_final)
  lasso_coeffs <- as.matrix(lasso_fit$fit$beta)

  saveRDS(
    list(model = lasso_final, metrics = lasso_metrics, coefficients = lasso_coeffs),
    file = slim_lasso_path
  )

  message("Saved slimmed LASSO model as lasso_model_minimal.rds")
} else {
  message("Slimmed LASSO model already exists. Skipping save.")
}
...
The slim model can be toggled on or off here by changing from TRUE to FALSE if wanting to use the full version. The full RDS file is saved in the large-files folder which will not automatically upload to github.
```{r}
## ---- load_lasso_model, message=TRUE, warning=FALSE -------------------

use_slim_lasso_model <- TRUE  # Toggle to FALSE to use full model

if (use_slim_lasso_model) {
  if (!file.exists(here::here("results", "output", "lasso_model_minimal.rds"))) {
    stop("Slim LASSO model not found. Please run the slim-save chunk or disable slim mode.")
  }

  message("Loading slimmed LASSO model...")
  lasso_slim <- readRDS(here::here("results", "output", "lasso_model_minimal.rds"))

  lasso_final_model <- lasso_slim$model
  lasso_metrics <- lasso_slim$metrics
  lasso_coeffs <- lasso_slim$coefficients
  best_lasso_params <- readRDS(here::here("results", "output", "lasso_best_params.rds"))

} else {
  if (!file.exists(here::here("results", "large-files", "lasso_tuned.rds"))) {
    stop("Full LASSO model not found.")
  }

  message("Loading full LASSO model...")
  lasso_tuned <- readRDS(here::here("results", "large-files", "lasso_tuned.rds"))
  lasso_metrics <- readRDS(here::here("results", "output", "lasso_tuned_metrics.rds"))
  lasso_final_model <- extract_workflow(lasso_tuned)

  lasso_fit <- extract_fit_parsnip(lasso_final_model)
  lasso_coeffs <- as.matrix(lasso_fit$fit$beta)
